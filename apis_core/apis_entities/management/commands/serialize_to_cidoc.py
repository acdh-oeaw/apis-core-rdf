...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if objcts.count() > 1000 and not options["use-cache"]:
            ...
            while (cnt * 1000) < objcts.count():
                ...
                for e in objcts[1000 * cnt : (1000 * cnt + 1000)]:
                    r.append(
                        EntitySerializer(
                            e, only_published=options["only-published"]
                        ).data
                    )
                with open(f"serializer_cache/{cnt}.pkl", "wb") as out:
                    ...
                    self.stdout.write(
                        self.style.NOTICE(
                            f"Pickle written to: serializer_cache/{cnt}.pkl"
                        )
                    )
                cnt += 1
            ...
            for e in objcts:
                res.append(
                    EntitySerializer(e, only_published=options["only-published"]).data
                )
        ...
        graph2 = Graph(store, identifier=URIRef(named_graph_vocabs))
        v_res_ser = []
        for v in ContentType.objects.filter(app_label="apis_vocabularies").exclude(
            model__in=["vocabnames"]
        ):
        ...
        fin_vocab = graph2
        fin, store = EntityToCIDOC().render(
            res,
            format_1=options["format"],
            store=store,
            binary=True,
        ...
        if options["output"]:
            ...
            if fin_vocab:
                with open(
                    f"{options['output'].split('.')[0]+'_vocabs.'+options['output'].split('.')[1]}",
                    "wb",
                ) as out2:
        ...
        if options["update-vocabs"]:
            ...
            if isinstance(vocabs_settings, dict):
                if "triplestore" in vocabs_settings.keys():
                    ...
                    sparql_serv_skosmos.setCredentials(
                        vocabs_ts_user, vocabs_ts_password
                    )
                    ...
                    if options["delete"]:
                        ...
                        count = int(
                            res_count_1["results"]["bindings"][0]["triples"]["value"]
                        )
                        if count > 0:
                            self.stdout.write(
                                self.style.NOTICE(
                                    f"Found {count} triples in skosmos named graph >> deleting"
                                )
                            )
                        params = {"graph": f"<{named_graph_vocabs}>"}
                        res3 = requests.delete(
                            f"{vocabs_ts_url}/graph",
                    ...
                    if count > 0:
                        self.stdout.write(
                            self.style.NOTICE(
                                f"Found {count} triples after update in store"
                            )
                        )
                    if res2.status_code != 200:
                        self.stdout.write(
                            self.style.ERROR(
                                f"Something went wrong when updating: {res2.text}"
                            )
                        )
                    else:
                        self.stdout.write(
                            self.style.SUCCESS("Updated the triplestore.")
                        )
                else:
                    self.stdout.write(
                        self.style.ERROR(
                            "You need to specify settings for the SKOSMOS triplestore"
                        )
                    )
        ...
        if options["update"]:
            ...
            if not options["triplestore"]:
                url, username, password = getattr(
                    settings, "APIS_BLAZEGRAPH", (False, False, False)
                )
                if not url:
                    self.stdout.write(
                        self.style.ERROR(
                            "When asking for update you need to either specify settings in APIS_BLAZEGRAPH or in the management command (--triple-store)"
                        )
                    )
                    return
            else:
                ...
                if not url or not username or not password:
                    self.stdout.write(
                        self.style.ERROR("Missing some settings for Triplestore update")
                    )
                    return
            ...
            if options["delete"]:
                ...
                if count > 0:
                    self.stdout.write(
                        self.style.NOTICE(
                            f"Found {count} triples in named graph >> deleting"
                        )
                    )
            ...
            if count > 0:
                self.stdout.write(
                    self.style.NOTICE(f"Found {count} triples after update in store")
                )
            if res2.status_code != 200:
                self.stdout.write(
                    self.style.ERROR(f"Something went wrong when updating: {res2.text}")
                )
            else:
