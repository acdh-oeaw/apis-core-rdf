    ...
    def handle(self, *args, **options):
        ...
        if options["includeall"]:
            ...
            if options["person"]:
                ...
                raise CommandError(
                    "You need to specify either a queryset for Persons or use --includeall"
                )
        if options["alltexts"]:
        ...
        with reversion.create_revision():
            for txt in tq[:2]:
                ...
                if not txt_type_new:
                    txt_type_new, created = TextType.objects.get_or_create(
                        name="default type resolved text"
                    )
                pers = txt.tempentityclass_set.all().values_list("pk", flat=True)
                pers = Person.objects.filter(pk__in=pers)
                if len(pers) != 1:
                    logging.warning(
                        "Could not identify a persons name. Resolving without person names."
                    )
                    pers_name = None
                else:
                    pers_name = [pers[0].first_name, pers[0].name]
                res = ResolveAbbreviations(
                    text=txt.text,
                    person=pers_name,
                    user=user_sketch,
                    password=password_sketch,
                )
