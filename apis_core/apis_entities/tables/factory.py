...
class EntitiesTableFactory:
    ...
    def get_table_class(cls, model_class:Model):
        ...
        if not table_class:
            ...
            table_class = cls.get_custom_table_class(model_name) or cls.create_new_entities_table_class(model_class, model_name)
        ...
        class GenericEntitiesTable(tables.Table):
            ...
            def __init__(self, visible_columns:tuple[str]=None, *args, **kwargs):
                ...
                if not visible_columns: 
                    warnings.warn(">>>>>>>> WARNING! You SHOULD set the param 'visible_columns', or the user will see all columns available for the model! <<<<<<<<")
                else:
