...
class GenericModelImporter:
    ...
    def create_instance(self):
        ...
        if not instance:
            ...
            logger.debug("Creating instance with attributes %s", attributes)
        ...
        for relation, details in data.get("relations", {}).items():
            ...
            related_model = ContentType.objects.get_by_natural_key(
                app_label=reld_app_label, model=reld_model
            ).model_class()

            for related_uri in details["curies"]:
                related = create_object_from_uri(uri=related_uri, model=related_model)
                if details.get("obj"):
                    attrs = {"subj": instance, "obj": related}
                else:
                    attrs = {"subj": related, "obj": instance}
                relation_model.objects.get_or_create(**attrs)
                logging.debug(
                    "Created relation %s between %s", relation_model.name, attrs
                )
        return instance
