...
class APISTestRunner(DiscoverRunner):
    def setup_databases(self, **kwargs):
        ...
        lst_cont = [
            ...
            .exclude(model__icontains="baseclass")
        ]
        lst_sort = [
            ct.objects.get(app_label="apis_metainfo", model="uri").model_class()
        ]
        ...
        while len(lst_cont) > len(lst_sort):
            for c in lst_cont:
                ...
                for fld in c._meta.get_fields() + c._meta.many_to_many:
                    if "_set" in fld.name or fld.name in [
                        "parent_class",
                        "vocab_name",
                        "userAdded",
                        "groups_allowed",
        ...
        for ent in lst_sort[1:]:
            ...
            for fld in ent._meta.get_fields():
                ...
                if fld.__class__.__name__ == "DateField" or ck:
                    ...
                    fld.__class__.__name__ != "ManyToManyField"
                    and fld.__class__.__name__ != "ManyToOneRel"
                ):
