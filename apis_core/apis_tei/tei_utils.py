...
from apis_core.utils.utils import get_child_classes
...
def get_context(res):
    ...
    if isinstance(res, Work):
        person_work_ids = get_child_classes(
            author_rels, PersonWorkRelation, labels=False
        )
        try:
...
def tei_header(
    title="ListPerson", ent_type="<listPerson/>", template_path="apis_tei/tei.xml"
):
    template = get_template(template_path)
