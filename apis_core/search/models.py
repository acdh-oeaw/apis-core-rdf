...
from .utils import (
    get_search_serialization_from_instance,
    split_query_string_for_search,
)
    ...
    def search(self, query_str: str, content_types=[], with_content=False):
        ...
        for weight, part in zip(weights, parts):
            for lookup, value in lookups.items():
                q |= Q(**{lookup: part})
                w += [When(**{lookup: part, "then": value + weight})]
...
class SearchEntry(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            ...
            GinIndex(
                name="generic_sea_content_text_gin",
                fields=["content_text"],
                opclasses=["gin_trgm_ops"],
            ),
            GinIndex(
                name="generic_sea_title_gin",
                fields=["title"],
                opclasses=["gin_trgm_ops"],
            ),
