...
def save_ajax_form(
    ...
    entity_instance_other = model_other_class_class.get_or_create_uri(
...
def create_triple_by_form_data(triple_form_data):
    ...
    if triple_form_data["triple_id"] != "":
        ...
        if len(triple_set) == 0:
            ...
            triple = triple_set[0]
    if (
        triple_form_data["property_id"] != ""
        and triple_form_data["model_other_id"] != ""
    ):
        model_self_class = caching.get_ontology_class_of_name(
            triple_form_data["model_self_class"]
        )
        model_self_instance = model_self_class.objects.get(
            pk=triple_form_data["model_self_id"]
        )
        ...
        if (
            triple_form_data["property_direction"]
            == PropertyAutocomplete.SELF_SUBJ_OTHER_OBJ_STR
        ):
            if triple is not None:
                ...
                triple = Triple.objects.get_or_create(
                    subj=model_self_instance, obj=model_other_instance, prop=property
                )[0]
        elif (
            triple_form_data["property_direction"]
            == PropertyAutocomplete.SELF_OBJ_OTHER_SUBJ_STR
        ):
            if triple is not None:
                ...
                triple = Triple.objects.get_or_create(
                    subj=model_other_instance, obj=model_self_instance, prop=property
                )[0]
        else:
...
def ajax_2_load_triple_form(request):
    ...
    should_include_other_entity = parse_boolean_val(
        request, "should_include_other_entity"
    )
    should_include_remove_button = parse_boolean_val(
        request, "should_include_remove_button"
    )
    should_include_create_button = parse_boolean_val(
        request, "should_include_create_button"
    )
    model_self_class = caching.get_ontology_class_of_name(
        request.POST["model_self_class"]
    )
    model_self_id = request.POST["model_self_id"]
    if model_self_id != "":
        model_self_instance = model_self_class.objects.get(
            pk=request.POST["model_self_id"]
        )
    else:
...
def ajax_2_post_reification_form(request):
    ...
    def process_reification_instance(post_data):
        reification_class = caching.get_reification_class_of_name(
            post_data["reification_type"]
        )
        reification_instance_id = post_data["reification_attr_form"].pop(
            "reification_id"
        )
        if reification_instance_id != "":
            reification_instance = reification_class.objects.get(
                pk=reification_instance_id
            )
        else:
    ...
    def process_triples_to_reification(reification_instance, triple_form_data_list):
        ...
        for triple_form_data in triple_form_data_list:
            if triple_form_data["model_other_id"] != "" and triple_form_data[
                "model_other_id"
            ] != str(reification_instance.pk):
                raise Exception(
            ...
            triple_form_data[
                "model_other_id"
            ] = reification_instance.uri_set.first().uri
    ...
    def process_triples_from_reification(reification_instance, triple_form_data_list):
        related_triple_list = []
        for triple_form_data in triple_form_data_list:
            if triple_form_data["model_self_id"] != "" and triple_form_data[
                "model_self_id"
            ] != str(reification_instance.pk):
                raise Exception(
                    "A wrong id was passed. The 'entity_other' in this case must be the reification"
    ...
    try:
        ...
        related_triple_list_before = Triple.objects.filter(
            Q(subj=reification_instance) | Q(obj=reification_instance)
        )
        related_triple_list_after = process_triples_to_reification(
            reification_instance, post_data["triple_data_to_reification_list"]
        )
        related_triple_list_after += process_triples_from_reification(
            reification_instance, post_data["triple_data_from_reification_list"]
        )
        for triple in related_triple_list_before:
            if triple not in related_triple_list_after:
                triple.delete()
    except FormException as e:
        response = JsonResponse(data={"error": str(e)}, status=500, safe=False)
    else:
        response = JsonResponse(
            data={
