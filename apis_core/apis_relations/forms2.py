...
class GenericRelationForm(forms.ModelForm):
    ...
    def get_html_table(self, entity_type, request, site_instance, form_match):
        ...
        print("apis_core/apis_relations/forms2.py")
        raise Exception()
...
class TripleForm(forms.ModelForm):
    ...
    def __init__(self, siteID=None, highlighter=False, *args, **kwargs):
        ...
        #                         args=[''.join([lst_src_target[0].lower(), lst_src_target[1].lower(), 'relation']),
        #                               'normal']),
        #             attrs=attrs))
        #     self.fields['target'] = autocomplete.Select2ListCreateChoiceField(
        #         label=lst_src_target[1],
        ...
        #                 args=[''.join([lst_src_target[0].lower(), lst_src_target[1].lower(), 'relation']), 'normal']
        #             ),
        #             attrs=attrs))
        #     self.fields['target'] = autocomplete.Select2ListCreateChoiceField(
        #         label=lst_src_target[1],
        ...
        self.rel_accessor = None
        self.fields["prop"] = autocomplete.Select2ListCreateChoiceField(
            label="prop",
            widget=ListSelect2(
                url=reverse(
                    "apis:apis_vocabularies:generic_vocabularies_autocomplete",
                    args=["vocabsbaseclass", "normal"]
                )
            )
        )
